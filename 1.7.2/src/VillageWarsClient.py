...
log.info('Running VillageWars ' + __version__)
...
icon = conf.images['Skins/0.png']
skins = [conf.res('Skins/%s.png' % (i), (280, 280)) for i in range(len(os.listdir(PATH + 'assets/Skins')) // 2)]

click_sound = conf.audio['click.wav']

title = conf.res('title page/attempt 1.png', (500, 100))
title_rect = title.get_rect(midtop=(500, 30))

SigningIn = conf.images['SigningIn.png']
CreatingAcount = conf.images['CreatingAcount.png']
...
def loading_circle(screen, clock):
    ...
    circle = conf.images['title page/loading.png']
    circle_angle = 0
    circle_speed = 5
...
def server_surfs(IPs, screen, choose_server_rect):
    ...
    for server in IPs:
        ...
        if ip.upper() == ip.lower():  # If the IP is numerical
            ...
            online_text = active + ' Online'
            ip_surf = font_server.render(online_text, True, (0,0,90))
            ip_surf2 = font_server.render(online_text, True, (50,50,50))
            ip_surf3 = font_server.render(online_text, True, (0,0,0))
...
def download_version_info(timeout=2):
    ...
    if conf.ctx.internet_connection:
        ...
        if conf.ctx.need_version_info(active):
            ...
            bar(100)
            del bar
            log.debug('Successfully downloaded version information')
